...
int main(int argc, char *argv[])
{
  ...
  MPI_Status send_status[NUM_KV_PER_RANK];
  ...
  for (i = 0; i < NUM_KV_PER_RANK; i++) { 
    ...
    MPI_Isend(&my_kv[i], 8, MPI_CHAR, (my_rank + (i + 1))        % size, 0, MPI_COMM_WORLD, &send_requests[i]);
    send_msg_count[i]++;
  }
  ...
  while (!is_finished()) {
	...
	/* MPI_Wait(&req, &stat); */
	while (!flag) MPI_Test(&send_requests[recv_index], &flag, &send_status[recv_index]);
	MPI_Isend(&sendrecv_kv, 8, MPI_CHAR, (my_rank + (recv_index + 1)) % size, 0, MPI_COMM_WORLD, &send_requests[recv_index]);
	send_msg_count[recv_index]++;
      }
    }
  }

  MPI_Waitall(NUM_KV_PER_RANK, send_requests, send_status);
